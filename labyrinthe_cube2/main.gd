	...
	if random.size() != 0:
		for i in (wall.size()):
			if random.pick_random() == 0:
				Global.lab[wall[0]] = Global.lab[Global.cote+1]
			wall.remove_at(0)
			
	Global.lab[pow(Global.cote, 2)+Global.cote] = Global.lab[pow(Global.cote, 2)+Global.cote+1]
	Global.lab[pow(Global.cote, 3)-1-pow(Global.cote, 2)-Global.cote] = Global.lab[pow(Global.cote, 3)-2-pow(Global.cote, 2)-Global.cote]
		...
		for k in Global.cote :
			for m in Global.cote :
				...
				if Global.lab[pos_in_lab] ==-1:
					var wall = (load("res://wall.tscn")as PackedScene).instantiate()
